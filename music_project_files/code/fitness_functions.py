...
def calculate_fitness_modulate(population, from_bar, to_bar, from_key, to_key, is_complex = True):
    ...
    if is_complex:
        ...
        for iPop in range(population_size):
            ...
            #when multiplying the default bias we change the rewlative weight of that function
...
def calculate_fitness_harmony(population, input_melody, key, counter = False):
    ...
    for iPop in range(population_size):
        ...
        fitness += more_calc(contrapunctal_motion_values['Oblique'],                default_bias)
        fitness += more_calc(contrapunctal_motion_values['Similar'],                default_bias)
